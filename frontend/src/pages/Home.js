...
export function Home() {
  ...
  const fetchRuns = async () => {
    try {
      const response = await api.get('/api/runs?limit=2');
            ...
            <CardContent className="p-0">
              <div className="border-b">
                <div className="flex gap-6 px-6 py-3">
                  <button className="text-sm font-medium text-primary border-b-2 border-primary pb-3">Recent</button>
                  <button className="text-sm text-muted-foreground hover:text-foreground pb-3">Scheduled</button>
                </div>
              </div>
                    ...
                    {runs.length > 0 ? (
                      runs.map((run) => (
                        <tr key={run.runId} className="border-b hover:bg-muted/50 cursor-pointer">
                          ...
                          <td className="p-4">
                            <span className="text-blue-600 dark:text-blue-400 font-medium">{run.resultCount}</span>
                          </td>
